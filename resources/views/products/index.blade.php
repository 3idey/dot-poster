...
<x-layout>
    ...
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-4xl font-bold text-gray-900">Discover Posters</h1>
        <div class="text-gray-600">{{ $products->total() }} posters available</div>
    </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="text" name="search" placeholder="Search posters..." 
                    ...
                    class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-900 placeholder-gray-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors">
            </div>
            
            {{-- Category Filter --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Category</label>
                <select name="category" class="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors">
            ...
            <div>
                ...
                <div class="flex space-x-2">
                    <select name="sort" class="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors">
    ...
    <div class=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        @foreach ($products as $p)
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 group">
                <a href="{{ route('products.show', $p->id) }}" class="block">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2 group-hover:text-emerald-600 transition-colors">{{ $p->name }}</h3>
                </a>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-2xl font-bold text-emerald-600">${{ number_format($p->price, 2) }}</span>
                    ...
                    @if($p->reviews->count() > 0)
                        <div class="flex items-center space-x-1 text-sm text-gray-600">
                            <div class="flex text-yellow-400">
                                @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-4 h-4 {{ $i <= round($p->average_rating) ? 'fill-current' : 'text-gray-600' }}" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                @endfor
                            </div>
                            <span>{{ number_format($p->average_rating, 1) }} ({{ $p->total_reviews }})</span>
                        </div>
                    @endif
                    ...
                    @auth
                        ...
                        <div class="flex items-center space-x-2 mb-3">
                            <button data-wishlist-button
                                    ...
                                    class="p-2 rounded-lg border transition-all duration-200 {{ auth()->user()->wishlists()->where('product_id', $p->id)->exists() ? 'text-red-500 border-red-500 hover:bg-red-500/10' : 'text-gray-400 border-gray-300 hover:border-red-500 hover:text-red-500' }}"
                                ...
                                <svg class="w-5 h-5 wishlist-heart" fill="{{ auth()->user()->wishlists()->where('product_id', $p->id)->exists() ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
